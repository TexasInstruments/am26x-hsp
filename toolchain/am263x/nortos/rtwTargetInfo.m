...
if strcmpi(computer('arch'), 'win64')
    config(end).Name                = 'TI ARM Clang Compiler AM263X | gmake';
    config(end).Alias               = 'TI ARM Clang Compiler AM263X | gmake';
    config(end).TargetHWDeviceType	= {'*'};
    config(end).FileName        = fullfile(toolsDir, 'ti_arm_clang_gmake_win64_AM263X.mat');
