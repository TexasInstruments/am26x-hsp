...
classdef MCSPI_CONTROLLER_TRANSFER_AM263X  < matlab.System & ...
    ...
    properties (Constant, Hidden)
        MCSPI_InstanceSet = matlab.system.StringSet({'MCSPI0', 'MCSPI1', 'MCSPI2', 'MCSPI3', 'MCSPI4'});
    end
    ...
    methods
        % Constructor
        function obj = MCSPI_CONTROLLER_TRANSFER_AM263X(varargin)
    ...
    methods (Static, Access=protected)
        ...
        function header = getHeaderImpl()
            header = matlab.system.display.Header(...
                'Title','MCSPI_CONTROLLER_TRANSFER',...
                'Text', sprintf('%s\n%s','Configure the MCSPI Controller Transfer block'),...
    ...
    methods (Access=protected)
        function setupImpl(obj) %#ok<MANU>
            if isempty(coder.target)
                ...
                coder.cinclude('MW_MCSPICONTROLLERTX.h');
        ...
        function output_msg = stepImpl(obj, u)  %#ok<INUSD>
            if isempty(coder.target)
                % Place simulation output code here 
            else
                % Call C-function implementing device output
                %coder.ceval('sink_output',u);
                msg = uint16(zeros(1024,1));
                InputLen = numel(u);
                OutputLen = uint16(0);
                switch (class(u))
                    case 'uint8'
                ...
                mcspi_controller_tx_step_struct = struct('base', obj.baseaddr, 'mcspi_instance', obj.MCSPI_InstanceEnum,'Enable_CH0', obj.Enable_Channel0, 'Enable_CH1', obj.Enable_Channel1, 'CH0_DataSize', obj.Channel0_Data_Size, 'CH1_DataSize', obj.Channel1_Data_Size, 'CH0_CSDisable', obj.Channel0_Auto_CSDeassert, 'CH1_CSDisable', obj.Channel1_Auto_CSDeassert, 'dataType', obj.DataType, 'output_data_length', uint32(obj.Output_Data_Length));
                coder.cstructname(mcspi_controller_tx_step_struct, 'MCSPIControllerTxStepStruct', 'extern', 'HeaderFile', 'MW_MCSPICONTROLLERTX.h');
                coder.ceval('MCSPICONTROLLERTX_step', coder.ref(mcspi_controller_tx_step_struct), uint32(InputLen), coder.ref(u), coder.wref(OutputLen), coder.wref(msg));
            
                output_msg = uint16(msg(1:obj.Output_Data_Length));
            end
        end
        ...
        function releaseImpl(obj) %#ok<MANU>
            if isempty(coder.target)
                ...
                coder.ceval('MCSPICONTROLLERTX_release');
            end
        end
    ...
    methods (Access=protected)
        ...
        function icon = getIconImpl(~)
            ...
            icon = {'MCSPI_CONTROLLER_TRANSFER'};
        end
    end
    ...
    methods (Static)
        function name = getDescriptiveName()
            name = 'MCSPI_CONTROLLER_TRANSFER';
        end
        ...
        function updateBuildInfo(buildInfo, context)
            if context.isCodeGenTarget('rtw')
                ...
                addIncludeFiles(buildInfo, 'MW_MCSPICONTROLLERTX.h', includeDir);
                addSourceFiles(buildInfo, 'MW_MCSPICONTROLLERTX.c', srcDir);
